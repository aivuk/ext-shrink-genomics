...
//   Dados = fopen("callithrixGLM.txt", "r");
   Dados = fopen("dados.T.csv", "r");
   ...
   Cov_Ext = (double **) malloc (tr*sizeof(double));
   for ( j = 0; j < tr; j++)
      Cov_Ext[j] = (double *) malloc (tr*sizeof(double));
   
   Corr_Ext = (double **) malloc (tr*sizeof(double));
   for ( j = 0; j < tr; j++)
      Corr_Ext[j] = (double *) malloc (tr*sizeof(double));
   ...
   for (j = 0; j < tr; j++)
      for ( k = 0, l = 0; k < Ne; k=k+2, l++)
         data[l][j] = (data[k][j] + data[k+1][j])/2;
   
   Ne /= 2;
   
   print_mat(data, "dadosMed.T.csv", Ne, tr);
   
   mat_cov (data, Ne, tr, Covar, Corr, Star);

   bend(Covar, Cov_Ext, tr);

   print_mat(Covar, "Covar.T.csv", tr, tr);
   print_mat(Corr, "Corr.T.csv", tr, tr);
   print_mat(Star, "Star.T.csv", tr, tr);
   print_mat(Cov_Ext, "Cov_Ext.T.csv", tr, tr);
